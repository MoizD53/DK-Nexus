  ...
  return (
    <div className="px-5 py-6 lg:px-8 lg:py-8 max-w-7xl mx-auto">
      <div className="mb-7">
        ...
        <p className="text-sm text-stone-400 mt-1">Discover exercises and perfect your form.</p>
      </div>
      ...
      {exercises.length === 0 ? (
        <div className="mt-8 bg-stone-900 border border-stone-800 rounded-xl overflow-hidden">
          <EmptyState 
            ...
            description={q || category ? "Try clearing your search or selecting a different category." : "Check back later when your gym adds exercises."} 
          />
        </div>
      ) : (
        <ExerciseGrid exercises={exercises} basePath="/member/exercises" />
      )}
    </div>
  );
}
